...
const ROWS_PER_BATCH: usize = 1_000_000; // Much smaller batches to reduce memory usage
const S3_CHUNK_SIZE: usize = 32 * 1024 * 1024; // 8MB S3 read buffer (reduced)
const MAX_BATCH_MEMORY: usize = 400 * 1024 * 1024; // 100MB max per batch
...
pub async fn stream_csv_to_parquet_multipart(
    bucket: &str,
    key: &str,
    ...
    let mut buffer = Vec::with_capacity(50 * 1024 * 1024);
